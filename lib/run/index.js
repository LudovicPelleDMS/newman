...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        // if options.packagesDirectory is provided, we need to resolve the packages
        let script;

        if (options.packagesDirectory && options.packagesPrefix) {
            // check if a packages directory exist in the working directory
            const packagesDirectory = `${options.workingDir}/${options.packagesDirectory}`,
                exist = fs.existsSync(packagesDirectory);

            if (!exist) {
                callback(new Error(`${packagesDirectory} does not exist`));

                return;
            }

            script = {
                packageResolver (_options, callback) {
                    // read the content of the packages directory
                    const packages = fs.readdirSync(packagesDirectory),
                        // console.log("FOUND PACKAGES: ", packages);
                        resolvedPackages = {};

                    // iterate over the packages and read the content of each file
                    packages.forEach((pkg) => {
                        const data = fs.readFileSync(`${packagesDirectory}/${pkg}`, 'utf8');

                        resolvedPackages[`${options.packagesPrefix}/${pkg.replace('.js', '')}`] = { data };
                    });
                    // console.log('RESOLVED PACKAGES: ', Object.keys(resolvedPackages));
        ...
        runner.run(options.collection, {
            ...
            function wrapScriptWithAsyncClosure (event) {
                if (event &&
                    (event.listen === 'test' || event.listen === 'prerequest') &&
                    event.script &&
                    event.script.exec &&
                    event.script.exec.length &&
                    event.script.exec[0] !== '(async function() {'
                ) {
                    event.script.exec.unshift('(async function() {');

                    // The async closure's promise was discarded: `})();` with no .catch().
                    // Any rejection inside it (a helper calling reject(), a throw after an
                    // await) became an UNHANDLED REJECTION. Node >= 15 defaults to
                    // --unhandled-rejections=throw, so the whole process died: newman never
                    // got to report the script error, and the run produced no summary table,
                    // no junit.xml and no html report. Measured on a real suite: the run was
                    // killed after 534 passing assertions, and all of them were lost.
                    //
                    // Catching is only half of it. A script that died must NOT become a green
                    // test, so the failure is re-surfaced as a failing assertion and, for a
                    // prerequest script, the request is skipped rather than sent against a
                    // half-built fixture.
                    event.script.exec.push('})().catch((e) => {');
                    event.script.exec.push('  console.error("[async script error] " + ((e && e.stack) || e));');
                    event.script.exec.push('  try { pm.test("async script error", function () { throw e; }); }');
                    event.script.exec.push('  catch (ignored) { /* pm.test unavailable in this scope */ }');
                    event.script.exec.push('  try { pm.execution && pm.execution.skipRequest &&');
                    event.script.exec.push('        pm.execution.skipRequest(); }');
                    event.script.exec.push('  catch (ignored) { /* skipRequest unavailable here */ }');
            ...
            emitter.on('beforeScript', function (err, o) {
                if (o && o.event) {
                    o.event = wrapScriptWithAsyncClosure(o.event);
                }
                // bubble special script name based events
